...
function validateFormData(data: ContactFormData): { isValid: boolean; errors: string[] } {
  ...
  if (!data.subject || data.subject.trim().length === 0) {
    ...
    if (subjectWordCount < 2) {
      errors.push(`Subject must contain at least 2 words (currently: ${subjectWordCount} words)`)
    }
  }
  ...
  if (!data.message || data.message.trim().length === 0) {
    ...
    if (wordCount < 5) {
      errors.push(`Message must contain at least 5 words (currently: ${wordCount} words)`)
    }
  }
